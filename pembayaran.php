...
<div id="atas">
	<div class="left">
		...
		<form method="get">
			...
			Cari bulan :<input type="number" name="bln" class="inputext" maxlength="2" max="12" min="1" style="width: 35px;" value="1"> dan tahun :<input type="number" name="thn" class="inputext" maxlength="4" style="width: 50px;" value="2022"><input type="submit" name="cari" value="Cari" class="button">
		</form>
		...
		<div class="right"><br>
			<?php if ($_SESSION['level'] == '100' or $_SESSION['level'] == '3' or $_SESSION['level'] == '2' && isset($_GET['serach'])): ?>
				<a href="pembayaran.php?hstr=1" class="button">History pembayaran</a>
			<?php endif ?>
			<?php if (isset($_GET['search'])): ?>
			<a href="pelanggan.php?search=<?= $_GET['search'] ?>" class="button">Kembali</a>
			<?php elseif(isset($_GET['bln'])): ?>
				<a href="pembayaran.php?hstr=1&serach=<?= $_GET['serach'] ?>" class="button">Kembali</a>
			<?php elseif(isset($_GET['serach'])): ?>
				<a href="pelanggan.php?search=<?= $_GET['serach'] ?>" class="button">Kembali</a>
			<?php else: ?>
				<a href="pelanggan.php" class="button">Kembali</a>
			<?php endif ?>
			</div>
			<div class="clear"></div>
			</div>
				...
				while ($byran = mysqli_fetch_assoc($mt)) {
					...
					<tr>
						...
						<td>
							...
							<a href="detail-bayar.php?byr=<?= $byran['id_penggunaan'] ?>" class="button">Detail / Bayar</a>
							<?php else: ?>
							atas dulu
							<?php endif ?>
						</td>
			...
			//$mt = select_pembayar();
					while ($byran = mysqli_fetch_assoc($mt)) {
						...
						<tr>
							...
							<td>
								<a href="print.php?meter=<?= $byran['no_meter'] .'&id='. $byran['id_penggunaan'] ?>" class="button">
									Print
								</a>
							</td>
						</tr>
